...
perl -0ne 'exit 0 if /ifneq \(\$\(findstring emcc,\$\(CC\)\),\)\s+PROJECT_LDFLAGS[[:space:]]*=.*-sASSERTIONS=1.*-sGL_ASSERTIONS=1.*-sMIN_WEBGL_VERSION=2.*-sMAX_WEBGL_VERSION=2\s+endif/s; exit 1' "$config_file"
...
! grep -Eq 'tokenizarPathSVG|leerNumeroPath|esTokenComando|esComandoPath' "$app_file"
